...
const modalCodePattern = /\{\/\* ── RENDER SETTINGS POPUP ── \*\/\}\s+\{showRenderSettings && \([\s\S]+?\)\}/g;
content = content.replace(modalCodePattern, '');
...
`;
    // Insert after the </div> line (lastIndex + 10 approximately)
    const result = content.slice(0, lastIndex + 10) + modalToInsert + content.slice(lastIndex + 10);
    fs.writeFileSync(filePath, result);
    console.log('Successfully cleaned App.tsx');
} else {
