...
// Keep everything before the corrupted map
const header = lines.slice(0, startIdx + 1).join('\n');

const body = `                  <input type="text" value={f.label} onChange={e => updateTemplateFieldDef(editingTemplate.id, f.id, { label: e.target.value })} placeholder={\`필드 \${idx + 1} 이름\`} style={{ background: "#18181b", border: \`1px solid \${BORDER}\`, borderRadius: 6, color: "#fff", padding: "6px 8px", fontSize: 11 }} />
          ...
          <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#000", position: "relative" }}>
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                <div style={{ width: "90%", height: "90%", background: "#111", borderRadius: 4, overflow: "hidden", position: "relative", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
                  ...
                  <div style={{ position: "absolute", top: 20, right: 20, background: "rgba(0,0,0,0.5)", padding: "4px 12px", borderRadius: 4, fontSize: 10, color: "#fff" }}>FR: 23.976</div>
                  <div style={{ position: "absolute", bottom: 40, left: "50%", transform: "translateX(-50%)", color: "#f59e0b", fontSize: 18, fontWeight: 800, textShadow: "0 2px 4px rgba(0,0,0,0.5)" }}>00:14:22:12</div>
                </div>
              </div>
            ...
            <div style={{ width: 340, background: "#141414", borderLeft: "1px solid #27272a", padding: 24, overflowY: "auto" }}>
              <div style={{ marginBottom: 32 }}>
                <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>📄 파일 요약</div>
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 10, color: "#71717a", marginBottom: 6 }}>파일명</div>
                  <input type="text" value={exportSettings.filename} onChange={e => setExportSettings(s => ({ ...s, filename: e.target.value }))} style={{ width: "100%", background: "#1c1c1e", border: "1px solid #27272a", borderRadius: 6, color: "#fff", padding: "10px 12px", fontSize: 12, outline: "none" }} />
                </div>
                <div>
                  <div style={{ fontSize: 10, color: "#71717a", marginBottom: 6 }}>저장 위치</div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ flex: 1, background: "#1c1c1e", border: "1px solid #27272a", borderRadius: 6, color: "#52525b", padding: "10px 12px", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{exportSettings.path}</div>
              ...
              <div style={{ marginBottom: 32 }}>
                <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>🎛 인코더 설정</div>
                <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input type="number" value={exportSettings.width} onChange={e => setExportSettings(s => ({ ...s, width: Number(e.target.value) }))} style={{ width: "100%", background: "#1c1c1e", border: "1px solid #27272a", borderRadius: 6, color: "#fff", padding: "10px 12px", fontSize: 12, outline: "none" }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: "#71717a", marginBottom: 6 }}>높이</div>
                    <input type="number" value={exportSettings.height} onChange={e => setExportSettings(s => ({ ...s, height: Number(e.target.value) }))} style={{ width: "100%", background: "#1c1c1e", border: "1px solid #27272a", borderRadius: 6, color: "#fff", padding: "10px 12px", fontSize: 12, outline: "none" }} />
                  </div>
                </div>
                <div style={{ marginBottom: 20 }}>
                  ...
                  <input type="range" min="1" max="100" step="0.1" value={exportSettings.bitrate} onChange={e => setExportSettings(s => ({ ...s, bitrate: Number(e.target.value) }))} style={{ width: "100%", accentColor: "#3b82f6" }} />
                </div>
              ...
              <div style={{ marginTop: "auto", paddingTop: 20, borderTop: "1px solid #27272a" }}>
                ...
                <button onClick={() => { startActualRender(); setIsExportView(false); }} style={{ width: "100%", padding: 14, background: "transparent", color: "#3b82f6", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 800, cursor: "pointer" }}>렌더링 시작</button>
              </div>
            </div>
          </div>
          <div style={{ height: 180, background: "#0a0a0a", borderTop: "1px solid #27272a", display: "flex", padding: 20, gap: 20 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", gap: 8 }}>📋 렌더 대기열</div>
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "flex-end" }}>
              <button onClick={() => { startActualRender(); setIsExportView(false); }} style={{ padding: "16px 40px", background: "#f59e0b", color: "#000", border: "none", borderRadius: 10, fontSize: 16, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>렌더 시작 ▶</button>
            </div>
          </div>
        </div>
      )}
