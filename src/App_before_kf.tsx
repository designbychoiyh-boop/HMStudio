...
export default function VibeEdit() {
  ...
  useEffect(() => {
    ...
    const clip = clips.find(c => time >= c.ts && time < c.ts + c.dur) ?? (time === 0 ? clips[0] : null);
    if (!clip) {
      vid.pause();
      vid.removeAttribute("src");
      vid.removeAttribute("data-cid");
      vid.load();
      return;
    }
    ...
    } else if (Math.abs(vid.currentTime - ct) > 0.15) {
      try { vid.currentTime = ct; } catch {}
    }
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      if (!rect) return;
      const g = graphics.find(g => g.id === interact.gid);
      if (!g) return;
      if (interact.mode === "move") {
        const dx = ((e.clientX - interact.px) / rect.width) * 100;
        const dy = ((e.clientY - interact.py) / rect.height) * 100;
        setGraphics(gs => gs.map(gg => gg.id === interact.gid ? { ...gg, x: clamp(interact.sx + dx, 0, 100), y: clamp(interact.sy + dy, 0, 100) } : gg));
        ...
        setGraphics(gs => gs.map(gg => gg.id === interact.gid ? { ...gg, scale: ns } : gg));
        ...
        while (next < -180) next += 360;
        setGraphics(gs => gs.map(gg => gg.id === interact.gid ? { ...gg, rotation: next } : gg));
      }
    };
  ...
  // ── Helpers ────────────────────────────────────────────────────────────
  const beginInteract = useCallback((e, g, mode) => {
    e.stopPropagation();
    const rect = stageRef.current?.getBoundingClientRect();
    if (!rect) return;
    const ct = time - g.ts;
    ...
    setSelGfxId(g.id); setSelClipId(null);
    setInteract({ mode, gid: g.id, px: e.clientX, py: e.clientY, sx, sy, ss, sr, sd, sa });
  }, [time]);
  ...
  const handleFileUpload = async e => {
    ...
    for (const file of files) {
      ...
      const dur = await new Promise(res => {
        ...
        v.onloadedmetadata = () => res(v.duration || 5); v.onerror = () => res(5);
      });
      // auto set composition to first video
      if (newClips.length === 0 && clips.length === 0) {
        const v2 = document.createElement("video"); v2.src = url; v2.preload = "metadata";
        await new Promise(res => { v2.onloadedmetadata = res; v2.onerror = res; });
        if (v2.videoWidth) setComp(c => ({ ...c, w: v2.videoWidth, h: v2.videoHeight }));
      }
      const clip = { id: uid(), file, url, name: file.name, dur, ts: t, startT: 0, endT: dur, opacity: 1, scale: 100, x: 50, y: 50, rotation: 0, track: 1 };
      newClips.push(clip);
      t += dur;
    }
    snap();
  ...
  const previewClip = clips.find(c => time >= c.ts && time < c.ts + c.dur) ?? (time === 0 ? clips[0] : null);
              ...
              {previewClip ? (
                <>
                  <video ref={videoRef} playsInline muted
                    style={{ position: "absolute", left: `${lerp(previewClip.kf?.x, time - previewClip.ts, previewClip.x)}%`, top: `${lerp(previewClip.kf?.y, time - previewClip.ts, previewClip.y)}%`, width: "100%", height: "100%", objectFit: "contain", opacity: lerp(previewClip.kf?.opacity, time - previewClip.ts, previewClip.opacity), transform: `translate(-50%,-50%) scale(${lerp(previewClip.kf?.scale, time - previewClip.ts, previewClip.scale) / 100}) rotate(${lerp(previewClip.kf?.rotation, time - previewClip.ts, previewClip.rotation ?? 0)}deg)`, transformOrigin: "center center", pointerEvents: "none" }}
                  />
            ...
            {selGfx ? (
              <>
                ...
                <div>
                  ...
                  <PropRow label="위치 X" value={Math.round(selGfx.x * 10) / 10} min={0} max={100} step={0.1} unit="%"
                    onChange={v => updateGfx(selGfx.id, { x: v })} onCommit={snap} />
                  <PropRow label="위치 Y" value={Math.round(selGfx.y * 10) / 10} min={0} max={100} step={0.1} unit="%"
                    onChange={v => updateGfx(selGfx.id, { y: v })} onCommit={snap} />
                  <PropRow label="비율 (Scale)" value={Math.round(selGfx.scale)} min={10} max={500} step={1} unit="%"
                    onChange={v => updateGfx(selGfx.id, { scale: v })} onCommit={snap} />
                  <PropRow label="회전" value={Math.round((selGfx.rotation || 0) * 10) / 10} min={-180} max={180} step={0.1} unit="°"
                    onChange={v => updateGfx(selGfx.id, { rotation: v })} onCommit={snap} />
                  <PropRow label="불투명도" value={Math.round(selGfx.opacity * 100)} min={0} max={100} step={1} unit="%"
              ...
              <>
                ...
                <PropRow label="위치 X" value={Math.round(selClip.x * 10) / 10} min={0} max={100} step={0.1} unit="%"
                  onChange={v => setClips(cs => cs.map(c => c.id === selClip.id ? { ...c, x: v } : c))} onCommit={snap} />
                <PropRow label="위치 Y" value={Math.round(selClip.y * 10) / 10} min={0} max={100} step={0.1} unit="%"
                  onChange={v => setClips(cs => cs.map(c => c.id === selClip.id ? { ...c, y: v } : c))} onCommit={snap} />
                <PropRow label="비율 (Scale)" value={Math.round(selClip.scale)} min={10} max={500} step={1} unit="%"
                  onChange={v => setClips(cs => cs.map(c => c.id === selClip.id ? { ...c, scale: v } : c))} onCommit={snap} />
                <PropRow label="회전" value={Math.round((selClip.rotation || 0) * 10) / 10} min={-180} max={180} step={0.1} unit="°"
                  onChange={v => setClips(cs => cs.map(c => c.id === selClip.id ? { ...c, rotation: v } : c))} onCommit={snap} />
                <PropRow label="불투명도" value={Math.round(selClip.opacity * 100)} min={0} max={100} step={1} unit="%"
                  onChange={v => setClips(cs => cs.map(c => c.id === selClip.id ? { ...c, opacity: v / 100 } : c))} onCommit={snap} />
              </>
            ) : (
