...
const autoFitLottieBackground = (data, fields = []) => {
  if (!data) return data;
  const assetMap = new Map((data?.assets || []).map(asset => [asset.id, asset]));
  const charMap = new Map((data?.chars || []).map(ch => [ch.ch, Number(ch.w || 0)]));
  const glyphChars = getGlyphChars(data);
  const textLayers = (data?.layers || []).filter(layer => layer?.ty === 5);
  const imageLayer = (data?.layers || []).find(layer => layer?.ty === 2 && assetMap.get(layer.refId));
  if (!textLayers.length || !imageLayer) return data;
  const asset = assetMap.get(imageLayer.refId);
  if (!asset || Number(asset.w || 0) <= 0) return data;
  const [scaleX] = getLayerScalePair(imageLayer);
  const currentWidth = Number(asset.w || 0) * (scaleX / 100);
  let desiredWidth = currentWidth;

  fields.filter(field => field?.bindingKey?.startsWith("__main__::")).forEach(field => {
    const width = shouldUseOverlayForField(field, glyphChars)
      ? measureCanvasTextWidth(field.value, field.fontFamily, field.fontSize, field.fontWeight, field.letterSpacing)
      : measureTextLineWidth(String(field.value || "").replace(/\n/g, ""), { s: field.fontSize || 72 }, charMap);
    const padding = Math.max(120, Number(field.fontSize || 72) * 2.4);
    desiredWidth = Math.max(desiredWidth, width + padding);
  });

  if (desiredWidth > currentWidth + 1) {
    scaleLayerX(imageLayer, desiredWidth / currentWidth);
  }
  return data;
};
...
const extractVectorSubtitleModel = data => {
  const mainLayers = data?.layers || [];
  const textLayers = mainLayers.filter(layer => layer?.ty === 5);
  const imageLayers = mainLayers.filter(layer => layer?.ty === 2);
  const isSubtitleLike = Number(data?.h || 0) <= 180 && textLayers.length === 1 && imageLayers.length === 1;
  if (!isSubtitleLike) return null;
  const textLayer = textLayers[0];
  const imageLayer = imageLayers[0];
  const assetMap = new Map((data?.assets || []).map(asset => [asset.id, asset]));
  const asset = assetMap.get(imageLayer.refId);
  if (!asset || Number(asset.w || 0) <= 0 || Number(asset.h || 0) <= 0) return null;
  const textDoc = getLottieTextDoc(textLayer);
  if (!textDoc) return null;
  const textPos = readTransformValue(textLayer?.ks?.p, [0, 0, 0]);
  const textScale = readTransformValue(textLayer?.ks?.s, [100, 100, 100]);
  const imagePos = readTransformValue(imageLayer?.ks?.p, [0, 0, 0]);
  const imageScale = getLayerScalePair(imageLayer);
  const barWidth = Number(asset.w || 0) * (Number(imageScale[0] || 100) / 100);
  const barHeight = Number(asset.h || 0) * (Number(imageScale[1] || 100) / 100);
  const imageScaleKeys = Array.isArray(imageLayer?.ks?.s?.k) && typeof imageLayer.ks.s.k[0] === 'object' ? imageLayer.ks.s.k : [];
  const textOpacityKeys = Array.isArray(textLayer?.ks?.o?.k) && typeof textLayer.ks.o.k[0] === 'object' ? textLayer.ks.o.k : [];
  return {
    sourceW: Number(data?.w || 1200),
    sourceH: Number(data?.h || 100),
    centerX: Number(imagePos?.[0] || 0),
    centerY: Number(imagePos?.[1] || 0),
    baseBarWidth: barWidth,
    baseBarHeight: barHeight,
    baseTextX: Number(textPos?.[0] || 0),
    baseTextY: Number(textPos?.[1] || 0),
    imageSrc: String(asset.p || ''),
    fontSize: Number(textDoc.s || 33) * (Number(textScale?.[0] || 100) / 100),
    strokeWidth: Number(textDoc.sw || 0) * (Number(textScale?.[0] || 100) / 100),
    lineHeight: Number(textDoc.lh || textDoc.s || 33) * (Number(textScale?.[1] || 100) / 100),
    textAlign: lottieJustifyToAlign(textDoc.j),
    accentStart: '#0E8D95',
    accentEnd: '#37F5F6',
    bodyFill: '#2B353D',
    bodyStroke: '#37F5F6',
    barAnimStart: Number(imageScaleKeys[0]?.t || 0) / Math.max(1, Number(data?.fr || 30)),
    barAnimEnd: Number(imageScaleKeys[imageScaleKeys.length - 1]?.t || 0) / Math.max(1, Number(data?.fr || 30)),
    textAnimStart: Number(textOpacityKeys[0]?.t || 0) / Math.max(1, Number(data?.fr || 30)),
    textAnimEnd: Number(textOpacityKeys[textOpacityKeys.length - 1]?.t || 0) / Math.max(1, Number(data?.fr || 30)),
  };
};
...
const applyLottieTextFields = (sourceData, fields = []) => {
  ...
  autoFitLottieBackground(cloned, fields);
  return cloned;
};
...
function TemplateTextOverlayField({ field }) {
  ...
  const textAnchor = anchorMode ? (textAlign === "left" ? "end" : textAlign === "right" ? "start" : "middle") : (textAlign === "left" ? "start" : textAlign === "right" ? "end" : "middle");
  ...
  return (
    <div style={{ position: "absolute", left: `${safeBox.x}%`, top: `${safeBox.y}%`, width: `${safeBox.w}%`, height: `${safeBox.h}%`, overflow: "visible", pointerEvents: "none" }}>
...
function TemplateTextOverlay({ fields = [] }) {
  if (!fields?.length) return null;
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "visible" }}>
      {fields.map(field => <TemplateTextOverlayField key={field.id} field={field} />)}
    </div>
  );
}
  ...
  if (resolvedLottieData) {
    ...
    return (
      <CroppedTemplateStage sourceW={template?.templateW} sourceH={template?.templateH} cropBounds={template?.cropBounds}>
        ...
        {overlayFields.length > 0 && <TemplateTextOverlay fields={overlayFields} />}
      ...
      if (pkg.lottie) {
        try {
          ...
          newTemplates.push({
            ...
            allowFontChange: true,
            allowColorChange: true,
            templateKind: vectorModel ? "vector_subtitle" : "lottie",
