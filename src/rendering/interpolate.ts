...
export function lerpKeyframe(kfs: Array<{ t: number; v: number; easing?: string }> | undefined, time: number, fallback: number) {
  ...
  for (let i = 0; i < sorted.length - 1; i += 1) {
    ...
    if (time >= a.t && time <= b.t) {
      ...
      // Apply easing if the starting keyframe has ease
