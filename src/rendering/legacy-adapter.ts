import type { AETemplateLayer, ImageLayer, ProjectState, ShapeLayer, TextLayer, TimelineLayer, VideoLayer } from './project-types';
...
function mapImageClip(clip: any): ImageLayer {
  return {
    ...baseLayerFromLegacy(clip),
    type: 'image',
    name: clip.name || 'image',
    url: clip.url,
    serverUrl: clip.serverUrl || null,
    storedPath: clip.storedPath || null,
    sourceW: Number(clip.sourceW || 1920),
...
export function legacySceneToProjectState(input: LegacySceneInput): ProjectState {
  ...
  const clips = Array.isArray(input.clips) ? input.clips.map(c => c.type === 'image' ? mapImageClip(c) : mapClip(c)) : [];
